...
import { rgb565Buffer, formatDate } from './utils';
import { GOO_VERSION, GOO_MAGIC, GOO_DELIMITER, GOO_TAIL } from './magics';
import { loadPreview, loadSlice } from './loaders';
...
export class GooWriter {
    ...
    protected async *writeHeader(header: GooHeader) {
        yield * this.writer.string(header.generator.description, 32);
        yield * this.writer.string(header.generator.version, 24);
